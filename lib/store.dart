...
class TempStore extends Store {
  ...
  Future<void> load() async {
    certs.clear();
    certs.add(EUDigitalCovidCertificateFactory().fromQrCode(
...
class LocalStore extends Store {
  ...
  @override
  Future<void> save() async {
    var persist = certs.map((EUDigitalCovidCertificate cert) {
      return cert.qr;
    }).toList();
  ...
  Future<void> load() async {
    ...
    persist.forEach((qr) {
      if (qr is String) {
        certs.add(EUDigitalCovidCertificateFactory().fromQrCode(qr)!);
      }
    });
  }
}
...
abstract class Store {
  List<EUDigitalCovidCertificate> certs = [];
  ...
  Future<bool> add(String qrcode) async {
    var c = EUDigitalCovidCertificateFactory().fromQrCode(qrcode);
    if (c == null) {
      return false;
    }
    certs.add(c);
  ...
  Iterable<T> map<T>(T Function(EUDigitalCovidCertificate) toElement) {
    return certs.map(toElement);
  }

  Map<int, EUDigitalCovidCertificate> asMap() {
    return certs.asMap();
  }
}
